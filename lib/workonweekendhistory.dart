...
class WorkOnWeekendHistoryModel {
 ...
 WorkOnWeekendHistoryModel({
  ...
  factory WorkOnWeekendHistoryModel.fromJson(Map<String, dynamic> json) {
    return WorkOnWeekendHistoryModel(
     id: json['id']?.toString(), 
      refNo: json['refno'],
...
class WorkonWeekendHistoryPage extends StatefulWidget {
  @override
  State<WorkonWeekendHistoryPage> createState() => _WorkonWeekendHistoryPageState();
}

class _WorkonWeekendHistoryPageState extends State<WorkonWeekendHistoryPage> {
  ...
  Map<String, List<WorkOnWeekendHistoryModel>> statusWiseHistory = {
  'Pending': [],
  'Approved': [],
  'Review': [],
  'Cancel': [],
...
Future<void> fetchWorkOnWeekendHistory() async {
  debugPrint('🔹 FetchWorkOnWeekendHistory() started...');
  setState(() {
    isLoading = true;
  });
  ...
  try {
   ...
   String? locationid = prefs.getString('location_id');
      ...
      try {
        ...
        if (data['status'] == 'success') {
          ...
          rawHistory.forEach((key, value) {
            parsedHistory[key] = List<WorkOnWeekendHistoryModel>.from(
              value.map((item) => WorkOnWeekendHistoryModel.fromJson(item)),
            );
          });
    ...
    debugPrint("🔥 Exception in fetchWorkOnWeekendHistory: $e");
  } finally {
    setState(() {
      isLoading = false;
    });
    debugPrint('🔹 FetchWorkOnWeekendHistory() ended.');
  }
}
Future<void> cancelWorkOnWeekendRequest(String id, int index) async {
  try {
...
Future<void> _selectDate(bool isFrom) async {
  ...
  Widget buildCardItem(WorkOnWeekendHistoryModel item, String tab, int index) {
...
Builder(
                          ...
                          return AlertDialog(
                            ...
                            actions: [
                              ...
                              ElevatedButton(
                                ...
                                onPressed: () async {
                                  Navigator.pop(context);
                                  await cancelWorkOnWeekendRequest(item.id!, index);
  ...
  child: TabBarView(
    children: tabs.map((tab) {
      final items = statusWiseHistory[tab] ?? [];
      if (items.isEmpty) {
        return Center(
          child: Text(
            'No $tab records',
      ...
      return ListView.builder(
        ...
        itemBuilder: (context, index) {
          return buildCardItem(items[index], tab, index);
        },
      );
    }).toList(),
  ),
            ...
            Positioned(
              ...
              child: ElevatedButton.icon(
                onPressed: () {
                  Navigator.push(
                   context,
                   MaterialPageRoute(builder: (context) => WorkonWeekendEntryPage()),
                  );
                },
                icon: const Icon(Icons.add, color: Colors.white),
                label: const Text("Request Work on Weekend", style: TextStyle(color: Colors.white)),
                style: ElevatedButton.styleFrom(
                  backgroundColor: const Color(0xFF346CB0),
                  shape: RoundedRectangleBorder(
